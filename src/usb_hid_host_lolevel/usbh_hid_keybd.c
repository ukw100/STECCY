...
#else // fm: simplified version to work with raw key codes, don't need map tables
...
{
    USR_KEYBRD_Init ();
    // printf ("KEYBRD_Init: usb_hid_host_result=%d\n", usb_hid_host_result);
}

static void
KEYBRD_Decode (uint8_t * pbuf)
{
    usb_hid_host_keyboard_decode (pbuf);
}
