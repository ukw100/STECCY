...
void USB_OTG_BSP_Init(USB_OTG_CORE_HANDLE *pdev) {
	...
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_NOPULL; // or GPIO_PuPd_DOWN;                  // fm: original is GPIO_PuPd_NOPULL
	GPIO_Init(GPIOA, &GPIO_InitStructure);

#if 1 // fm
	GPIO_RESET_BIT(GPIOA, GPIO_Pin_11); // fm
	GPIO_RESET_BIT(GPIOA, GPIO_Pin_12); // fm
    delay_msec (100);                   // fm
#endif
...
void USB_OTG_BSP_DriveVBUS(USB_OTG_CORE_HANDLE *pdev, uint8_t state) {
    (void) pdev; // fm
#ifdef USB_HID_HOST_DISABLE_VBUS
    (void) state; // fm
#else
	if (0 == state) {
